...
module.exports = function (agenda) {
  agenda.define("Free Talk Thread", async function (job, done) {
    try {
      ...
      const last_thread = await getLastThread("Free Talk Thread");
      ...
      const doc = await renderTemplate("ftt", { data });
      const markdown = turndownService.turndown(doc, { gfm: true });
      const post_title = `[${date}] ${day} Free Talk Thread`;
